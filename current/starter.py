#!/usr/bin/env python3
"""个人 AI 开发项目启动器 V1.9.2。

用户通过自然语言把项目名称、目录、类型和简介告诉具备本地权限的项目角色，
由该角色内部调用本工具。备用双击入口仍保留，但不属于日常工作流。

V1.9.2 使用临时目录完成渲染、配置解析、Git 初始化、初始提交、Hook 安装和健康检查；
...
def validate_generated_project(target: Path) -> None:
    ...
    if data.get("template_version") != "1.9.2":
...
def initialize_git(target: Path) -> None:
    try:
        ...
        raise RuntimeError(f"Git 初始化失败；V1.9.2 项目治理模式要求 Git：{exc}") from exc
...
def create_initial_commit(target: Path) -> str:
    ...
    try:
        ...
        subprocess.run(
            ["git", "commit", "-m", "Initialize project from template V1.9.2"],
            cwd=target,
            check=True,
            capture_output=True,
            text=True,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="个人 AI 开发项目启动器 V1.9.2")
